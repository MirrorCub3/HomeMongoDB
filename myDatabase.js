...
myDatabase.prototype.postHome = function(home,res) { // the acessing the databse code
  let obj = {id:home.id,style:home.style,cost:home.cost,view:home.view}; // storing info into a js object
...
myDatabase.prototype.getHome = function(id,res) {
    // searches the database for the name ident and then an id that matches the given value
  HomeModel.find({id:id},function(error,info) { // findng the syudnt of tht particular id and getting a pointer to it -----> info
...
myDatabase.prototype.putHome = function(home,res) {
  ...
  HomeModel.findOneAndUpdate({id:home.id},{id:home.id,style:home.style,cost:home.cost,view:home.view},function(error,oldHome) {
    if (error) {
...
myDatabase.prototype.deleteHome = function(id,res) {
    HomeModel.remove({id:id},function(error,removed) {
        if (error) {
            return res.json({retVal:false});
        }
